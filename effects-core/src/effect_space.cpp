...
void put(Rgb *frame, const Matrix& matrix, int16_t x, int16_t y, Rgb color, bool additive = true) {
    if (y < 0 || y >= matrix.height()) return;
    Rgb& pixel = frame[matrix.xy_wrap(x, (uint8_t)y)];
    pixel = additive ? add_rgb(pixel, color) : color;
}

void disc(Rgb *frame, const Matrix& matrix, float cx, float cy, float radius, Rgb color,
          bool additive = true) {
    ...
    for (int16_t y = min_y; y <= max_y; y++) {
        for (int16_t x = min_x; x <= max_x; x++) {
            const float dx = x - cx;
            const float dy = y - cy;
            if (dx * dx + dy * dy <= limit) put(frame, matrix, x, y, color, additive);
        }
    }
}
...
void star_background(Rgb *frame, const Matrix& matrix, float t, Rgb background,
    ...
    for (uint8_t y = 0; y < matrix.height(); y++) {
        for (uint8_t x = 0; x < matrix.width(); x++) {
            ...
            const float twinkle =
                0.5f + 0.5f * std::sin(t * (1.1f + (seed & 7) * 0.13f) + seed * 0.17f);
            const uint8_t value = to_byte(65.0f + twinkle * 175.0f);
            put(frame, matrix, x, y, Rgb::from_hsv((uint8_t)(145 + seed % 55), 80, value));
        }
    }
}
...
void render_nebula(Rgb *frame, const Matrix& matrix, uint32_t now_ms) {
    for (uint8_t y = 0; y < matrix.height(); y++) {
        for (uint8_t x = 0; x < matrix.width(); x++) {
            const uint8_t a =
                cylindrical_noise8(x, y, matrix.width(), 125, (uint16_t)(now_ms / 10));
            const uint8_t b = cylindrical_noise8(matrix.wrap_x(x + 4), y, matrix.width(), 83,
                                                 (uint16_t)(now_ms / 16));
...
void render_wormhole(Rgb *frame, const Matrix& matrix, float t) {
    ...
    for (uint8_t y = 0; y < matrix.height(); y++) {
        for (uint8_t x = 0; x < matrix.width(); x++) {
            ...
            float stripe = 0.5f + 0.5f * std::sin(radius * 2.55f - t * 3.1f + angle * 4.0f);
            stripe *= stripe;
            const float aperture = clamp01((radius - 1.25f) / 2.2f);
            const uint8_t value = to_byte(stripe * aperture * 255.0f);
            const uint8_t hue =
                (uint8_t)((angle + kPi) * (256.0f / (2.0f * kPi)) + radius * 14.0f + t * 28.0f);
...
void render_orbits(Rgb *frame, const Matrix& matrix, float t) {
    ...
    for (uint8_t i = 0; i < 3; i++) {
        ...
        disc(frame, matrix, px, py, i == 1 ? 1.15f : 0.8f, Rgb::from_hsv(hues[i], 220, 245));
    }
}
...
void render_solar_storm(Rgb *frame, const Matrix& matrix, float t, uint32_t now_ms) {
    for (uint8_t y = 0; y < matrix.height(); y++) {
        for (uint8_t x = 0; x < matrix.width(); x++) {
            ...
            const float ribbon = std::sin(y * 0.82f + std::sin(angle * 2.0f + t) * 2.1f - t * 1.7f);
            const uint8_t noise =
                cylindrical_noise8(x, y, matrix.width(), 105, (uint16_t)(now_ms / 5));
            float energy = clamp01((ribbon + 0.35f) * 0.78f);
            energy = clamp01(energy + (noise - 128) / 350.0f);
            const uint8_t hue = (uint8_t)(2 + energy * 38.0f);
            frame[matrix.xy(x, y)] = Rgb::from_hsv(hue, 245, to_byte(12 + energy * 243));
        }
    }
}
...
void render_pulsar(Rgb *frame, const Matrix& matrix, float t) {
    ...
    for (uint8_t y = 0; y < matrix.height(); y++) {
        for (uint8_t x = 0; x < matrix.width(); x++) {
            ...
            const float beams =
                clamp01(1.0f - std::fabs(dx) / 1.1f) + clamp01(1.0f - std::fabs(dy) / 1.1f);
            const float core = clamp01(1.0f - radius / (1.2f + pulse));
            const uint8_t value = to_byte(ring * 150 + beams * pulse * 75 + core * 255);
            frame[matrix.xy(x, y)] = Rgb::from_hsv((uint8_t)(145 + radius * 4), 165, value);
        }
    }
}
...
void render_asteroid(Rgb *frame, const Matrix& matrix, float t, uint32_t now_ms) {
    ...
    if (phase < 0.55f) {
        ...
        for (uint8_t i = 1; i < 8; i++) {
            ...
            put(frame, matrix, (int16_t)(ax + i * 0.65f), (int16_t)(ay + i * 0.9f),
                Rgb::from_hsv((uint8_t)(18 - i), 250, value));
        }
        ...
        for (uint8_t y = 0; y < matrix.height(); y++) {
            for (uint8_t x = 0; x < matrix.width(); x++) {
                ...
                put(frame, matrix, x, y, Rgb::from_hsv((uint8_t)(8 + p * 25), 235, value));
            }
        }
    }
}
...
void render_pacman(Rgb *frame, const Matrix& matrix, float t, uint32_t now_ms) {
    ...
    for (int8_t oy = -2; oy <= 2; oy++) {
        for (int8_t ox = -2; ox <= 2; ox++) {
            ...
            put(frame, matrix, (int16_t)(px + ox + 0.5f), (int16_t)(py + oy + 0.5f), {255, 210, 0},
                false);
        }
    }
}
...
void render_space_police(Rgb *frame, const Matrix& matrix, float t) {
    ...
    for (uint8_t y = 0; y < matrix.height(); y++) {
        for (uint8_t x = 0; x < matrix.width(); x++) {
            const float angle = 2.0f * kPi * x / matrix.width();
            const float sweep = 0.5f + 0.5f * std::sin(angle * 2.0f + y * 0.7f - t * 5.0f);
            ...
            frame[matrix.xy(x, y)] = {to_byte(red * 255), to_byte((red + blue) * 18),
                                      to_byte(blue * 255)};
        }
    }
}
...
void render_purple_meteors(Rgb *frame, const Matrix& matrix, float t) {
    ...
    for (uint8_t m = 0; m < 8; m++) {
        ...
        for (uint8_t i = 0; i < 9; i++) {
            ...
            put(frame, matrix, (int16_t)(head_x + i * 0.72f), (int16_t)(head_y + i * 0.72f),
...
void render_light_speed(Rgb *frame, const Matrix& matrix, float t) {
    ...
    for (uint8_t s = 0; s < 38; s++) {
        ...
        const float progress = std::fmod(t * (0.42f + (seed & 15) * 0.012f) + s * 0.071f, 1.0f);
        ...
        for (uint8_t i = 0; i < 5; i++) {
            ...
            put(frame, matrix, (int16_t)(cx + std::cos(angle) * r + 0.5f),
                ...
                Rgb::from_hsv((uint8_t)(145 + seed % 65), 105, (uint8_t)(255 - i * 48)));
        }
    }
}
...
void render_supernova(Rgb *frame, const Matrix& matrix, uint32_t now_ms) {
    ...
    if (phase < 0.2f) {
        ...
        disc(frame, matrix, cx, cy, radius, {to_byte(255 * p), to_byte(230 * p), to_byte(170 * p)});
        return;
    }
    ...
    for (uint8_t y = 0; y < matrix.height(); y++) {
        for (uint8_t x = 0; x < matrix.width(); x++) {
            ...
            put(frame, matrix, x, y, Rgb::from_hsv((uint8_t)(18 + p * 205), 205, value));
        }
    }
}
    ...
    for (uint8_t y = 0; y < matrix.height(); y++) {
        for (uint8_t x = 0; x < matrix.width(); x++) {
            ...
            const uint8_t terrain =
                cylindrical_noise8(x, y, matrix.width(), 181, (uint16_t)(now_ms / 30));
            const uint8_t value = to_byte((120 + terrain / 2) * fade);
            put(frame, matrix, x, y, Rgb::from_hsv((uint8_t)(4 + terrain / 32), 235, value), false);
        }
    }
}
...
void render_singularity(Rgb *frame, const Matrix& matrix, float t) {
    ...
    for (uint8_t y = 0; y < matrix.height(); y++) {
        for (uint8_t x = 0; x < matrix.width(); x++) {
            ...
            const float swirl = 0.5f + 0.5f * std::sin(angle * 5.0f - t * 4.0f + radius * 1.2f);
...
void render_relic(Rgb *frame, const Matrix& matrix, uint32_t now_ms) {
    for (uint8_t y = 0; y < matrix.height(); y++) {
        for (uint8_t x = 0; x < matrix.width(); x++) {
            const uint8_t broad =
                cylindrical_noise8(x, y, matrix.width(), 105, (uint16_t)(now_ms / 35));
            const uint8_t fine = cylindrical_noise8(matrix.wrap_x(x + 3), y, matrix.width(), 241,
                                                    (uint16_t)(now_ms / 28));
            const uint8_t signal = (uint8_t)(((uint16_t)broad * 3 + fine) / 4);
            const uint8_t hue =
                signal < 128 ? (uint8_t)(145 + signal / 4) : (uint8_t)((signal - 128) / 3);
...
void EffectSpace::tick(MatrixCanvas& canvas, const Matrix& matrix, const EffectParams&,
                       uint32_t now_ms) {
    ...
    switch (style_) {
    case SpaceStyle::CosmicNebula:
        render_nebula(frame, matrix, frame_time.total_ms);
        break;
    case SpaceStyle::Wormhole:
        render_wormhole(frame, matrix, t);
        break;
    case SpaceStyle::OrbitalDance:
        render_orbits(frame, matrix, t);
        break;
    case SpaceStyle::SolarStorm:
        render_solar_storm(frame, matrix, t, frame_time.total_ms);
        break;
    case SpaceStyle::Pulsar:
        render_pulsar(frame, matrix, t);
        break;
    case SpaceStyle::StarrySky:
        render_starry_sky(frame, matrix, t);
        break;
    case SpaceStyle::AsteroidImpact:
        render_asteroid(frame, matrix, t, frame_time.total_ms);
        break;
    case SpaceStyle::PacmanOrbit:
        render_pacman(frame, matrix, t, frame_time.total_ms);
        break;
    case SpaceStyle::SpacePolice:
        render_space_police(frame, matrix, t);
        break;
    case SpaceStyle::PurpleMeteors:
        render_purple_meteors(frame, matrix, t);
        break;
    case SpaceStyle::LightSpeed:
        render_light_speed(frame, matrix, t);
        break;
    case SpaceStyle::Supernova:
        render_supernova(frame, matrix, frame_time.total_ms);
        break;
    case SpaceStyle::MarsFlight:
        render_mars_flight(frame, matrix, t, frame_time.total_ms);
        break;
    case SpaceStyle::Singularity:
        render_singularity(frame, matrix, t);
        break;
    case SpaceStyle::RelicRadiation:
        render_relic(frame, matrix, frame_time.total_ms);
        break;
    }

    for (uint16_t i = 0; i < canvas.size(); i++)
        canvas.set_pixel(i, frame[i]);
}
