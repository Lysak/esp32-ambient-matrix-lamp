...
void EffectCampfire::tick(MatrixCanvas& canvas, const Matrix& matrix,
    ...
    // Smooth noise feeds the base while temporal interpolation gives the fire
    // enough inertia to look like a flame instead of random LED flicker.
    const uint16_t noise_time = (uint16_t)(now_ms / 4);
    ...
    Rgb frame[kMaxSize * kMaxSize]{};
    for (uint8_t y = 0; y < h; y++) {
        for (uint8_t x = 0; x < w; x++) {
            frame[matrix.xy(x, y)] = color_from_palette(kCampfireColors,
                                                        heat_[x][y], 255);
        }
    }
    ...
    for (uint16_t i = 0; i < count; i++) canvas.set_pixel(i, frame[i]);
}
