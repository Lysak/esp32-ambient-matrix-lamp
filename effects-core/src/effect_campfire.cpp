...
void EffectCampfire::tick(MatrixCanvas& canvas, const Matrix& matrix, const EffectParams&,
                          uint32_t now_ms) {
        ...
        for (uint8_t y = 1; y < h; y++) {
            for (uint8_t x = 0; x < w; x++) {
                ...
                const uint16_t rising =
                    (uint16_t)heat_[x][y - 1] * 3 + heat_[left_x][y - 1] + heat_[right_x][y - 1];
                const uint16_t lateral =
                    (uint16_t)heat_[x][y] * 2 + heat_[left_x][y] + heat_[right_x][y];
                const uint8_t mixed = (uint8_t)((rising * 3 + lateral) / 19);
                const uint8_t cooling =
                    (uint8_t)(4 + y * 2 + scale8(cylindrical_noise8(x, y, w, 121, noise_time), 5));
    ...
    for (uint8_t y = 0; y < h; y++) {
        for (uint8_t x = 0; x < w; x++) {
            frame_buf[matrix.xy(x, y)] = color_from_palette(kCampfireColors, heat_[x][y], 255);
        }
    }
    ...
    for (auto& spark : sparks_) {
        ...
        frame_buf[index] =
            add_rgb(frame_buf[index], color_from_palette(kCampfireColors, 238, spark.life));
    }
