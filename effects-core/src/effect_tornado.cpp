...
void EffectTornado::tick(MatrixCanvas& canvas, const Matrix& matrix, const EffectParams& params,
                         uint32_t now_ms) {
    ...
    for (uint8_t k = 0; k < layers; k++) {
        for (uint8_t y = 0; y < h; y++) {
            // k*10000 gives each layer an independent noise seed (GyverLamp2 original)
            uint8_t noise_val = inoise8((uint16_t)(y * 10) + t1 + (uint16_t)k * 10000u, t2);
            int16_t col = map_int(noise_val, 50, 200, 0, (int16_t)w);

            // Brightness falloff: 255, 170, 85, 0 (j * 85 step, GyverLamp2 original)
            static const uint8_t kWidths = 4;
            for (uint8_t j = 0; j < kWidths; j++) {
                uint8_t bright = 255 - (uint8_t)j * 85;
