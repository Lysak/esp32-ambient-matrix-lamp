...
void EffectFire2020::tick(MatrixCanvas& canvas, const Matrix& matrix, const EffectParams& params,
                          uint32_t now_ms) {
    ...
    if (params.scale != (uint8_t)last_scale_) {
    ...
    for (uint8_t x = 0; x < w; x++) {
        for (uint8_t y = 0; y < h; y++) {
            ...
            // random8(2) adds 0 or 1 jitter to y — matches GyverLamp2's subtle flicker
            uint8_t n = cylindrical_noise8(x, (uint8_t)(j + random8(2)), w, delta_value_,
    ...
    for (uint8_t i = 0; i < num_sparks; i++) {
        ...
        if (spark_height > 3 && spark_height < h) {
            ...
            uint8_t n3 = cylindrical_noise8(sx, (uint8_t)j3, w, delta_value_,
                                            (uint16_t)(ff_z_ + ff_y_ * delta_hue_));
            ...
            canvas.set_pixel(matrix.xy(sx, sy), color_from_palette(palette, idx3, 255 - fade));
        }
    }
}
