...
uint8_t inoise8(uint16_t x, uint16_t y) {
    ...
    return lerp8(lerp8(kP[a], kP[b], u), lerp8(kP[(uint8_t)(a + 1)], kP[(uint8_t)(b + 1)], u), v);
}
...
uint8_t cylindrical_noise8(uint8_t x, uint8_t y, uint8_t width, uint16_t spatial_scale,
                           uint16_t time) {
    if (width == 0) return 0;
    int32_t circle_x;
    int32_t circle_y;
