...
void EffectPatterns::tick(MatrixCanvas& canvas, const Matrix& matrix, const EffectParams&,
                          uint32_t now_ms) {
    ...
    for (uint8_t y = 0; y < h; y++) {
        for (uint8_t x = 0; x < w; x++) {
            ...
            case PatternStyle::Plasma: {
                ...
                const uint8_t hue = (uint8_t)((int)(wave * 30.0f) + (int)(frame.total_ms / 20));
                const uint8_t value = byte_from_float(185.0f + 55.0f * std::sin(wave + t * 0.4f));
            ...
            case PatternStyle::Aurora: {
                const float drift =
                    ((int)cylindrical_noise8(x, y, w, 170, (uint16_t)(frame.total_ms / 5)) - 128) /
                    70.0f;
                const float curtain = cy + std::sin(angle * 2.0f + t * 0.62f) * 2.2f + drift;
                const float distance = std::fabs(y - curtain);
                float glow = clamp01(1.0f - distance / 5.2f);
                glow *= glow;
                const uint8_t shimmer =
                    cylindrical_noise8(x, y, w, 110, (uint16_t)(frame.total_ms / 7));
                const uint8_t value = byte_from_float(glow * (145.0f + shimmer * 0.42f));
                const uint8_t hue =
                    (uint8_t)(112 + std::sin(angle + t * 0.2f) * 24.0f + frame.total_ms / 180);
            ...
            case PatternStyle::OceanWaves: {
                ...
                const uint8_t texture =
                    cylindrical_noise8(x, y, w, 105, (uint16_t)(frame.total_ms / 8));
                const uint8_t index =
                    byte_from_float(45.0f + normalized * 170.0f + texture * 0.16f);
                const uint8_t value = byte_from_float(85.0f + normalized * 165.0f);
                color = color_from_palette(kOceanColors, index, value);
                break;
            }

            case PatternStyle::LavaLamp: {
                const uint8_t n1 = cylindrical_noise8(x, y, w, 155, (uint16_t)(frame.total_ms / 7));
                const uint8_t n2 = cylindrical_noise8(matrix.wrap_x(x + 3), y, w, 83,
                                                      (uint16_t)(frame.total_ms / 11));
                const uint8_t blend = (uint8_t)(((uint16_t)n1 * 3 + n2) / 4);
                const uint16_t hot = blend > 82 ? (uint16_t)(blend - 82) * 3 / 2 + 28 : blend / 4;
                const uint8_t shaped = hot > 255 ? 255 : (uint8_t)hot;
                color = color_from_palette(kLavaColors, shaped, qadd8(80, shaped));
                break;
            }
            ...
            case PatternStyle::Kaleidoscope: {
                ...
                const float facets =
                    std::sin((mx + my) * 1.18f + t * 1.15f) +
                    std::sin(std::cos(angle * 3.0f) * 3.0f - my * 1.72f - t * 0.83f);
                const uint8_t hue = (uint8_t)((int)(facets * 42.0f) + (int)(frame.total_ms / 24));
                const uint8_t value = byte_from_float(150.0f + std::fabs(facets) * 52.0f);
                color = Rgb::from_hsv(hue, 245, value);
                break;
            }
            ...
            case PatternStyle::NeonRings: {
                ...
                const uint8_t hue = (uint8_t)(radius * 21.0f + frame.total_ms / 18);
